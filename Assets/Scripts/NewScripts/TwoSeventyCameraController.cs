...
public class TwoSeventyCameraController : MonoBehaviour
{
    
    public Rigidbody rb;
    public ParticleSystem movement_PS;

    [Header("Force Settings")]
    [Tooltip("Amount of impulse force applied when spacebar is pressed.")]
    ...
    void Update()
    {
        ...
        // 7 Apply forward force if the space bar is pressed
        if (Input.GetKey(KeyCode.Space))
        {
            rb.AddForce(transform.forward * forceAmount / 10f, ForceMode.Impulse);

            if (!movement_PS.isPlaying)
                movement_PS.Play();
        }
        else
        {
